...
\subsubsection{Timer <-> Fixed Priority Scheduler:}

\begin{itemize}
    \item \lstinline|ExTimerInit()|: No calls between components.
    
    \item \lstinline|ExTimerStop()|: No calls between components.
    
    \item \lstinline|ExTimerWait() <-> ThreadBlock()|: We don't use anymore the busy-waiting technique; we will call \lstinline|ThreadBlock()| to suspend the thread and remove it from the active execution list, allowing other threads to execute in the meantime.
    
    \item \lstinline|ExTimerUninit()|: No calls between components.
    
    \item \lstinline|ExTimerSystemPreinit(void)|: No calls between components.
    
    \item \lstinline|ExTimerCompareListElems(PLIST_ENTRY t1, PLIST_ENTRY t2 , PVOID context)|: No calls between components.
    
    \item \lstinline|ExTimerCheck(PEX_TIMER timer)|: No calls between components.
    
    \item \lstinline|ExTimerCheckAll(void) <-> ThreadUnblock()|: Examines all active timers; we will call \lstinline|ThreadUnblock()| to unblock the thread and add it back to the active execution list, where the priority scheduler will handle its execution based on priority.
\end{itemize}

\subsubsection{Timer <-> Priority Donation}

\begin{itemize}
    \item \lstinline|ExTimerInit()|: No calls between components.
    
    \item \lstinline|ExTimerStop()| <-> \lstinline|ThreadSetPriority()|: When a thread is blocked by a timer which expires, if its Real and Effective priorities are different, its priority will be reset to the Real priority.
    
    \item \lstinline|ExTimerWait()|: No calls between components.
    
    \item \lstinline|ExTimerUninit()|: No calls between components.
    
    \item \lstinline|ExTimerSystemPreinit()|: No calls between components.
    
    \item \lstinline|ExTimerCompareListElems()|: No calls between components.
    
    \item \lstinline|ExTimerCheck()|: No calls between components.
    
    \item \lstinline|ExTimerCheckAll(void)|: No calls between components.
\end{itemize}

\subsubsection{Fixed Priority Scheduler <-> Priority Donation}

\begin{itemize}
    \item \lstinline|ThreadSystemPreinit()|: No calls between components;
    
    \item \lstinline|ThreadYield()| <-> \lstinline|ThreadDonatePriority()|: When a thread wishes to yield to the CPU, the function will check if it has any mutexes and if any threads are waiting for it, in which case  it will donate the priority to those waiting.
    
    \item \lstinline|ThreadUnblock()|: <-> \lstinline|ThreadSetPriority()|: When a thread is unblocked, if its Real and Effective priorities are different, its priority will be reset to the Real priority.

    \item \lstinline|MutexAcquire()|: No calls between components.
    
    \item \lstinline|ExEventWaitForSignal()|: No calls between components.
    
    \item \lstinline|ThreadSetPriority()|: No calls between components.
    
    \item \lstinline|SmpSendGenericIpi()|: No calls between components.

    \item \lstinline|ThreadComparePriorityReadyList()|: No calls between components.
    
    \item \lstinline|ThreadYieldForIpi()|: No calls between components.
\end{itemize}
...
\begin{itemize}
  \item \lstinline|TestThreadTimerAbsolute()|: 
  Creates an absolute timer set to expire in 50\,ms, calculated using \lstinline|IomuGetSystemTimeUs()|. 
  Through \lstinline|ExTimerInit()|, \lstinline|ExTimerStart()|, and a single \lstinline|ExTimerWait()| call, the test verifies that the waiting thread resumes precisely when the absolute target time is reached.

  \item \lstinline|TestThreadTimerAbsolutePassed()|: 
  Sets an absolute timer to a timestamp that occurred 50\,ms in the past. 
  Since the deadline has already elapsed, \lstinline|ExTimerWait()| must return immediately. 
  This confirms that timers scheduled in the past are handled correctly and do not block the thread.

  \item \lstinline|TestThreadTimerPeriodicMultiple()|: 
  Configures a relative periodic timer with a 50\,ms interval and waits on it ten times in succession. 
  The thread calls \lstinline|ExTimerWait()| after each expiration, ensuring that periodic wakeups occur at accurate intervals across all cycles.

  \item \lstinline|TestThreadTimerPeriodicOnce()|: 
  Uses the same periodic setup as the previous test but waits only for the first expiration. 
  This isolates the validation of the initial periodic trigger, confirming that the thread resumes execution after a single 50\,ms delay.

  \item \lstinline|TestThreadTimerPeriodicZero()|: 
  Starts a relative periodic timer with a zero period. 
  Each call to \lstinline|ExTimerWait()| should complete instantly, demonstrating that the thread is never blocked and continues execution immediately.

  \item \lstinline|TestThreadTimerSharedTimer()|: 
  All worker threads (typically 16) share one relative once timer initialized in \lstinline|TestThreadTimerPrepare()|. 
  Each thread waits in \lstinline|ExTimerWait()|, and once the shared timer expires after 50\,ms, all threads should resume simultaneously.

  \item \lstinline|TestThreadTimerMultipleTimers()|: 
  Assigns each thread its own relative periodic timer, with the period set to \(50\,\mathrm{ms} + 10\,\mathrm{ms}\times i\) for thread \(i\). 
  The test checks that threads wake up in the correct order according to their individual timer intervals.

  \item \lstinline|TestThreadTimerRelative()|: 
  Creates a single relative once timer configured for a 50\,ms timeout. 
  The helper \lstinline|_TestThreadSleepWaitTimer()| performs a single \lstinline|ExTimerWait()| call to confirm that the thread resumes exactly after the expected delay.

  \item \lstinline|TestThreadTimerRelativeZero()|: 
  Programs a relative once timer with a zero-millisecond delay. 
  The call to \lstinline|ExTimerWait()| must return immediately, showing that a zero-duration timer never blocks the waiting thread.
\end{itemize}

\subsubsection{Priority Scheduler. Fixed Priority Scheduler}

\begin{itemize}
    \item \lstinline|TestThreadPriorityWakeup()|: 
    Launches several threads (typically 16) whose priorities range incrementally from \lstinline|ThreadPriorityLowest| upward. 
    Each thread waits on an \lstinline|EX_EVENT| object, and the purpose of the test is to confirm that the scheduler awakens the threads in order of descending priority; higher-priority threads should resume execution before lower-priority ones.

    \item \lstinline|TestThreadPriorityMutex()|:
    Performs the same validation as \newline \lstinline|TestThreadPriorityWakeup()|, but replaces event synchronization with \lstinline|MUTEX| objects. 
    The test ensures that when multiple threads of varying priorities contend for a mutex, they acquire it according to their priority ranking.

    \item \lstinline|TestThreadPriorityYield()|:
    Creates a single thread running at maximum priority and verifies that it remains continuously scheduled, even when it voluntarily calls \lstinline|ThreadYield()|. 
    The goal is to ensure that the scheduler does not preempt a top-priority thread that chooses to yield execution.

    \item \lstinline|TestThreadPriorityRoundRobin()|:
    Starts multiple threads (by default 16) all assigned the same highest priority, and checks that invoking \lstinline|ThreadYield()| causes the CPU to rotate fairly among them. 
    This confirms that threads sharing the same priority are scheduled in a round-robin manner.
\end{itemize}
...
\begin{itemize}
    \item \lstinline|TestThreadPriorityDonationOnce()|: Tests if a higher priority thread can donate its priority to a different thread which has acquired a mutex it is waiting for.
    
    \item \lstinline|TestThreadPriorityDonationLower()|: Follows the same structure as the previous test, however it checks that the thread which received the donation maintains it even when trying to lower its own priority.
    
    \item \lstinline|TestThreadPriorityDonationMulti()|: The main thread creates two mutexes which it holds initially, and two threads, A and B, which try to acquire those mutexes. After thread B is allowed to execute, the test checks if the main thread still has A's priority
    
    \item \lstinline|TestThreadPriorityDonationMultiInverted()|:  Same as the previous test, however the mutexes are released in the inverse order. The main thread should have B's priority.
    
    \item \lstinline|TestThreadPriorityDonationMultiThreads()|: Same as the previous test, however with three threads, A, B, and C. B and C have a higher priority, and are both waiting for the second mutex. After the second mutex is released, the main thread should have a priority equal to B/C's.
    
    \item \lstinline|TestThreadPriorityDonationMultiThreadsInverted()|: Same as the previous test, however the mutexes are released in the inverse order. The main thread should have B/C's priority.
    
    \item \lstinline|TestThreadPriorityDonationNest()|: Tests if nested threads can succesively donate their priorities.
    
    \item \lstinline|TestThreadPriorityDonationChain()|: Same as the previous test, however with more threads.
\end{itemize}
