...
Implement the system calls \lstinline|SyscallThreadExit()|, \lstinline|SyscallThreadCreate()|, \lstinline|SyscallThreadGetTid()|, \lstinline|SyscallThreadWaitForTermination()| and \newline \lstinline|SyscallThreadCloseHandle()|.
...
\textbf{File Editing:} A basic text editor requires access to the file system in order to create and update files. Various essential commands such as \lstinline|ls|, \lstinline|touch|, and \lstinline|rm| require access to the file system.

\noindent \textbf{Build System:} A compilation tool like \lstinline|make| needs to compile multiple source files in parallel to reduce build time. It uses system calls to spawn compiler processes for independent files, passes command-line arguments specifying input files and compiler flags, and uses wait operations to synchronize completion before linking.
...
\subsubsection*{Handle Model (per-process)}

We use \textbf{per-process} handle tables for \emph{process}, \emph{thread}, and \emph{file} objects. A handle is a small integer returned to user mode; the kernel validates it on every system call and checks that it belongs to the \textbf{current process} and matches the \textbf{expected type}. This keeps isolation simple.

\paragraph{Data structures (added headers).}
We add the following definitions to a new header \lstinline|headers/um_handle_manager.h| and include it where needed. Embed the table into \lstinline|PROCESS| (see below).
...
\subsubsection*{Member in PROCESS (added field).}
We add this to \lstinline|headers/executive/process_internal.h| inside the \lstinline|PROCESS| structure:

\begin{lstlisting}
UM_HANDLE_TABLE HandleTable;   // per-process handle table
\end{lstlisting}

\subsubsection*{Minimal handle manager (new source).}
Provide a small manager with the signatures below. These are thin list/lock helpers used by all syscalls.

\noindent \lstinline|STATUS UmHandleInitTable(PUM_HANDLE_TABLE T);| \newline
\noindent \lstinline|void UmHandleUninitTable(PUM_HANDLE_TABLE T);| \newline
\noindent \lstinline|STATUS UmHandleCreate(PUM_HANDLE_TABLE T, HANDLE_TYPE Type, PVOID Obj, DWORD* Out);| \newline
\noindent \lstinline|STATUS UmHandleResolve(PUM_HANDLE_TABLE T, DWORD H, HANDLE_TYPE Expect, PVOID* OutObj);| \newline
\noindent \lstinline|void UmHandleClose(PUM_HANDLE_TABLE T, DWORD H);| \newline

\subsubsection*{Functions for System Calls for Process Management}

Process syscalls create, resolve, and close \emph{per-process} handles. On any invalid handle, null/invalid pointer, or type mismatch, the handler returns an error.
...
\subsubsection*{Functions for System Calls for File System Access}
...
\begin{itemize}
 \item \lstinline|SyscallThreadExit(Status)|: Exist the currently running thread and handles resource cleanup. Notifies any waiting threads. Returns an exit status to the operating system.

 \item \lstinline|SyscallThreadCreate(ThreadHandle, Routine, Arg)|: Create a new thread inside the current running process. The thread will begin executing \textit{Routine} using the given \textit{Arg}. A handle will be returned inside the \textit{ThreadHandle} pointer. Returns \textit{0} on success.

 \item \lstinline|SyscallThreadGetTid(ThreadHandle, ThreadId)|: Get the TID for the given \textit{ThreadHandle} and store it inside \textit{ThreadId}.

 \item \lstinline|SyscallThreadWaitForTermination(ThreadHandle, Retval)|: Wait for the termination of \textit{ThreadHandle}. Its return value will be stored inside the \textit{Retval} pointer.

 \item \lstinline|SyscallThreadCloseHandle(ThreadHandle)|: Close the handle associated with \textit{ThreadHandle}
 \end{itemize}
...
The system call handlers for process management interact with the kernel functions for process management by invoking functions defined in \lstinline|process.h| and \lstinline|process.c|, such as \lstinline|ProcessCreate()|, \lstinline|ProcessWaitForTermination()|, \lstinline|ProcessGetId()|, and \lstinline|ProcessCloseHandle()|.
...
We first validate all user-supplied pointers (path, arguments, and output handle) across page boundaries and copy the strings into kernel memory. If the path is relative, we normalize it to the Applications folder on the system drive. We then ask the kernel loader to create the process, yielding a \lstinline|PPROCESS| on success. This object is registered in the \textbf{current process's} handle table by creating a new integer handle, associating it with the kernel object, and marking its type as \texttt{PROCESS}. The new handle is written back to user mode; any validation, normalization, or loader error is returned as a status code.

\noindent\textbullet\ \lstinline|SyscallProcessWaitForTermination()| \newline
We validate the output status pointer and then \textbf{resolve} the supplied handle in the \textbf{current process's} handle table under lock. We verify that the handle exists and that its type is \texttt{PROCESS}. After successful resolution, we use the kernel's wait mechanism on the target process's termination event, which blocks the calling thread until the process exits. We then obtain the final exit code and copy it back to the user. If the handle is not owned by the caller, is invalid, or has the wrong type, we fail the call without waiting.

\noindent\textbullet\ \lstinline|SyscallProcessGetPid()| \newline
There are two modalities. If the user passes the sentinel \texttt{INVALID} handle, we return the PID of the \textbf{current} process from the process hierarchy. Otherwise, we resolve the given handle in the \textbf{current process's} handle table and verify that its type is \texttt{PROCESS}. If resolution and type check succeed, we obtain the PID of the target process and return it to the user; otherwise the call fails.

\noindent\textbullet\ \lstinline|SyscallProcessCloseHandle()| \newline
A user closes a process handle they no longer need. We locate and resolve this handle in the \textbf{current process's} handle table, verify its type is \texttt{PROCESS}, then remove it from the table. We dereference the underlying kernel object so the kernel can reclaim it when the last reference is gone. If the handle is missing, not owned by the caller, or of the wrong type, we return an error.

\subsection*{System Calls for File System Access}

\noindent\textbullet\ \lstinline|SyscallFileCreate()| \newline
We validate the path and output handle pointers and copy the path into kernel memory. For a relative path, we interpret it relative to the system drive root. The user-mode stub enters the kernel, where the \lstinline|SyscallHandler| invokes the internal \lstinline|IoCreateFile| to open or create the file/directory and return a kernel file object on success. We then create a new integer handle in the \textbf{current process's} handle table, associate it with the file object, mark the type as \texttt{FILE}, and return this handle to user mode. Validation, normalization, or open/create failures are returned as status codes.

\noindent\textbullet\ \lstinline|SyscallFileRead()| \newline
We validate the output buffer and \lstinline|BytesRead| pointers and check the requested size. We resolve the supplied handle in the \textbf{current process's} handle table and verify that its type is \texttt{FILE}. If resolution succeeds, the kernel reads up to the requested byte count into the user buffer using safe copy helpers and updates \lstinline|BytesRead| with the actual number of bytes read. Invalid, non-owned, or mistyped handles cause the call to fail.

\noindent\textbullet\ \lstinline|SyscallFileClose()| \newline
We pass the file handle to the kernel and remove it from the \textbf{current process's} handle table after verifying it is valid and of type \texttt{FILE}. We decrement the file object's reference count; when no references remain, the kernel closes the file and releases its resources. If the handle is invalid, not owned by the caller, or has the wrong type, the close fails with an error.
